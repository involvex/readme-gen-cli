...
// ---------------------------------------------------------------------------
// readme-cli — Auto-generate a README.md from package.json and project structure
...
function printHelp() {
	const help = `
readme-cli — Auto-generate a README.md from your project

USAGE
  readme-cli [options] [directory]
...
EXAMPLES
  readme-cli                        Print README to stdout
  readme-cli -o README.md           Write to README.md
  readme-cli -o README.md --force   Overwrite existing README.md
  readme-cli --minimal              Generate minimal README
`
...
function loadGitignorePatterns(dir) {
	...
	try {
		...
		console.error(
			`readme-cli: failed to read .gitignore in ${dir}: ${e.message}`,
		)
	}
...
function buildTree(dir, ignorePatterns, prefix, maxDepth, currentDepth) {
	...
	try {
		...
		console.error(`readme-cli: failed to read directory ${dir}: ${e.message}`)
		return []
	}
...
function generateReadme(pkg, dir, opts) {
	...
	if (pkg.license)
		title += ` ![License](https://img.shields.io/badge/license-${pkg.license}-green)`
	sections.push(title)
	...
	// ---- Usage
	{
		...
		if (pkg.bin) {
			...
			usageLines.push('```js')
			usageLines.push(
				`const ${safeName(name)} = require('${pkg.name || name}');`,
			)
			usageLines.push('```')
		} else {
			usageLines.push('```js')
	...
	if (!opts.minimal) {
		...
		const structLines = ['## Project Structure', '', '```', tree, '```']
	...
	// ---- License
	{
		...
		if (pkg.license) {
			...
			licenseLines.push(
				`This project is licensed under the [${pkg.license} License](./LICENSE).`,
			)
			if (author) {
				licenseLines.push('')
...
function main() {
	...
	try {
		...
		if (err.code === 'ENOENT') {
			console.error(`readme-cli: no package.json found in ${dir}`)
			console.error(
				'Run readme-gen from a directory containing a package.json.',
			)
			process.exit(1)
		}
	...
	if (opts.output === null) {
		...
		if (fs.existsSync(outPath) && !opts.force) {
			console.error(
				`readme-cli: ${opts.output} already exists. Use --force to overwrite.`,
			)
			process.exit(1)
		}

		fs.writeFileSync(outPath, readme, 'utf8')
		console.error(`readme-cli: wrote ${opts.output} (${readme.length} bytes)`)
	} else {
